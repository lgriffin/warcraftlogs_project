...
class ConsumesAnalyzer:
    ...
    def analyze_raid(self, client: WarcraftLogsClient, report_id: str) -> None:
        ...
        logger.info("Analyzing consumables for raid: %s", report_id)

        # Get raid metadata
        metadata = get_report_metadata(client, report_id)
        self.raid_metadata[report_id] = {"title": metadata["title"], "date": metadata["start"]}
        ...
        for player in master_actors:
            ...
            try:
                ...
                try:
                    ...
                    cast_ids = [int(sid) for sid in self.config.get("cast_consumables", {})]
                ...
                logger.warning("Error analyzing %s: %s", player_name, e)
    ...
    def _fetch_boss_kills(self, client: WarcraftLogsClient, report_id: str) -> None:
        ...
        try:
            ...
            logger.warning("Could not fetch boss kills: %s", e)
    ...
    def _count_consumables_from_table(
        self, player_name: str, player_role: str, report_id: str, table_data: dict, cast_events: list[dict]
    ) -> None:
        """
    ...
    def _count_consumables(self, player_name: str, player_role: str, report_id: str, events: list[dict]) -> None:
        ...
        for event in events:
            if event.get("type") in ["applybuff", "cast", "begincast", "refreshbuff", "removebuff"]:
                ...
                ability_events[ability_id].append({"type": event_type, "timestamp": timestamp})
            ...
            if spell_id_int in ability_events:
                ...
                for event in ability_event_list:
                    if event["type"] in ["applybuff", "refreshbuff"]:
                        self.timestamp_data[report_id][spell_name].append(
                            {"timestamp": event["timestamp"], "player": player_name, "type": event["type"]}
                        )
    ...
    def _export_to_csv(self, output_file: str) -> None:
        ...
        logger.info("CSV export completed: %s", output_file)
    ...
    def export_to_markdown(self, output_path: str | None = None) -> str:
        ...
        if not output_path:
            ...
            safe_title = (
                "".join(c if c.isalnum() or c in " _-" else "_" for c in (titles[0] if titles else "consumes"))
                .strip()
                .replace(" ", "_")
            )
            from . import paths
...
def run_consumes_analysis(
    ...
    for raid_id in raid_ids:
        try:
            ...
            logger.error("Error analyzing raid %s: %s", raid_id, e)

    analyzer.generate_report(output_csv, include_healers=include_healers)

    if markdown_path:
        path = analyzer.export_to_markdown(None if markdown_path == "auto" else markdown_path)
