...
class SpellManager:
    ...
    def __init__(self, spell_data_dir: str | None = None):
        ...
        self._reverse_aliases: dict[int, set[int]] | None = None
                ...
                if isinstance(aliases, dict):
                    for source_id, canonical_id in aliases.items():
                        try:
                            ...
                            logger.warning("Invalid alias mapping %s:%s - %s", source_id, canonical_id, e)

        except (OSError, json.JSONDecodeError) as e:
            logger.error("Error loading spell aliases: %s", e)
                ...
                if isinstance(spells, dict):
                    for spell_id, spell_name in spells.items():
                        try:
                            ...
                            logger.warning("Invalid name mapping %s:%s - %s", spell_id, spell_name, e)

        except (OSError, json.JSONDecodeError) as e:
            logger.error("Error loading spell names: %s", e)
    ...
    def process_spell_events(self, events: list, exclude_ids: set[int] | None = None) -> dict[int, int]:
        """
    ...
    def get_fear_ward_usage(self, cast_entries: list) -> dict[str, Any] | None:
        """
    ...
    def validate_configuration(self) -> bool:
        ...
        try:
            ...
            logger.debug("Loaded %d spell aliases", len(aliases))
            logger.debug("Loaded %d spell names", len(names))
                ...
                if canonical_id in aliases and aliases[canonical_id] != canonical_id:
                    ...
                    while current in aliases and current not in visited:
                        ...
                        if current in path[:-1]:
                            logger.warning("Circular alias detected: %s", " -> ".join(map(str, path)))
                            return False
...
class SpellBreakdown:
    ...
    @staticmethod
    def get_fear_ward_usage(cast_entries: list) -> dict[str, Any] | None:
    ...
    def log_cleave_ids(events: list) -> None:
        ...
        manager = get_spell_manager()
        logger.debug("Scanning damage taken for known and unknown Cleave IDs")
